...
constexpr float LINE_SEARCH_FWD_PCT  = 18.0f;    // đi tới nhẹ khi search (tạo đà)
...
/** Điều khiển xe theo turn + forward (mix-steering chuẩn). */
inline void lineBotDrive(int16_t turn, int16_t fwdPct) {
  uint16_t baseSpd = g_state.baseSpeed;
  if (baseSpd == 0) baseSpd = (uint16_t)((uint32_t)PWM_MAX * fwdPct / 100u);
  // Mix: turn > 0 → right chậm hơn, left nhanh hơn
  // botDrive(turnCmd, fwdCmd, baseSpd) — arcade với curve
  botDrive(turn, fwdPct, baseSpd);
}
...
inline void lineDecoderUpdate(float dtS) {
  ...
  switch (g_ldState) {
    ...
    case LD_TRACKING: {
      ...
      // Gần node + → APPROACHING (debounce ≥2 frame)
      if (p == LINE_PAT_NODE && g_lineState.stableFrames >= 2) {
